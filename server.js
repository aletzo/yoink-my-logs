...
const dir =
  process.env.YOINK_LOG_DIR || path.join(os.homedir(), ".yoink-my-logs");
      ...
      if (location && typeof location === "object") {
        if (location.file && location.line) {
          ...
          logEntry.location = {
            ...
            absolutePath: location.fullPath
    ...
    try {
      ...
      if (!timestamp || !message) {
        ...
        res.end(
          JSON.stringify({
            success: false,
            error: "Missing timestamp or message",
          })
        );
        return;
      }
...
function followFile(res) {
  ...
  dirWatcher = fs.watch(dir, (eventType, filename) => {
    if (
      eventType === "rename" &&
      filename &&
      filename.startsWith(todayPrefix()) &&
