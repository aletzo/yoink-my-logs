...
describe("pushLog edge cases", () => {
  ...
  test("rejects log entries larger than 100KB", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 700}`))
      .default;
    ...
    assert.strictEqual(
      after,
      before,
      "should not write logs larger than 100KB"
    );
  });

  test("handles logs with special characters", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 701}`))
      .default;
  ...
  test("handles null and undefined values in data", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 702}`))
      .default;
  ...
  test("handles deeply nested data", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 703}`))
      .default;
    ...
    assert.strictEqual(
      log.data.level1.level2.level3.level4.level5.value,
      "deep"
    );
  });

  test("handles circular reference gracefully", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 704}`))
      .default;
  ...
  test("handles array data", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 705}`))
      .default;
  ...
  test("handles Date objects in data", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 706}`))
      .default;
...
describe("pushLog directory handling", () => {
  ...
  test("writes to correct file based on date", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 707}`))
      .default;
