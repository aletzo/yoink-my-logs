...
describe("/clear endpoint", () => {
  ...
  test("clears all logs from today's files", async () => {
    ...
    assert.strictEqual(
      beforeTestLogs.length,
      3,
      "should have 3 test logs before clearing"
    );
    ...
    assert.strictEqual(
      afterLines.length,
      0,
      "should have no logs after clearing"
    );
  });
...
describe("/delete endpoint", () => {
  ...
  test("deletes a specific log entry by timestamp and message", async () => {
    ...
    // Add some logs
    const yoink = (await import(`../../index.js?t=${Date.now() + 201}`))
      .default;
    ...
    assert.ok(
      afterLogs.some((log) => log.message === `${testId}-keep-1`),
      "should keep first log"
    );
    assert.ok(
      afterLogs.some((log) => log.message === `${testId}-keep-2`),
      "should keep second log"
    );
    assert.ok(
      !afterLogs.some((log) => log.message === `${testId}-delete-me`),
      "should not have deleted log"
    );
  });
  ...
  test("returns success: false when log is not found", async () => {
    ...
    assert.strictEqual(
      response.data.success,
      false,
      "should return success: false for non-existent log"
    );
  });
  ...
  test("deletes only the first matching log when duplicates exist", async () => {
    ...
    assert.strictEqual(
      afterLines.length,
      2,
      "should have 2 logs remaining after deleting one duplicate"
    );
  });
});
