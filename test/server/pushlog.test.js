...
describe("pushLog()", () => {
  ...
  test("logs are written in order when called rapidly", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      assert.strictEqual(
        testLogs[i].message,
        `${testId}-${i}`,
        `log ${i} should be in correct position`
      );
      assert.strictEqual(
        testLogs[i].data.index,
        i,
        `log ${i} should have correct index data`
      );
    }
  });
  ...
  test("logs with large data payloads maintain order", async () => {
    ...
    assert.strictEqual(
      testLogs.length,
      payloads.length,
      `should have ${payloads.length} test logs`
    );
    ...
    for (let i = 0; i < payloads.length; i++) {
      ...
      assert.strictEqual(
        testLogs[i].data.payload.size,
        payloads[i].size,
        `log ${i} should have correct payload`
      );
    }
  });
});
