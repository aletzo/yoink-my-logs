      ...
      if (
        ...
        try {
          ...
          if (pathname === "/" || pathname === "") {
            ...
            shortPath =
              url.hostname + (url.port ? ":" + url.port : "") + " (inline)";
          } else {
        ...
        shortPath = pathParts.length > 1 ? pathParts.slice(-2).join("/") : file;
      }
...
yoink.first = (first, second) =>
  sendSliced(first, second, (data) => {
    if (!Array.isArray(data)) return data;
    if (data.length === 0) return undefined;
    return data[0];
  });
yoink.five = (first, second) =>
  sendSliced(first, second, (data) => sliceArray(data, 0, 5));
yoink.ten = (first, second) =>
  sendSliced(first, second, (data) => sliceArray(data, 0, 10));

// yoink.last is both a function and has .five() and .ten() methods
yoink.last = (first, second) =>
  sendSliced(first, second, (data) => {
    if (!Array.isArray(data)) return data;
    if (data.length === 0) return undefined;
    return data[data.length - 1];
  });
yoink.last.five = (first, second) =>
  sendSliced(first, second, (data) => sliceFromEnd(data, 5));
yoink.last.ten = (first, second) =>
  sendSliced(first, second, (data) => sliceFromEnd(data, 10));
...
if (typeof window !== "undefined" && window.YOINK_REPLACE_CONSOLE_LOG) {
  ...
  typeof process !== "undefined" &&
  process.env &&
  process.env.YOINK_REPLACE_CONSOLE_LOG
) {
