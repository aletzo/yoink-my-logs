...
describe("SSE /events endpoint", () => {
  ...
  test("SSE endpoint returns correct headers", async () => {
    const response = await new Promise((resolve, reject) => {
      const req = http.get(
        `http://localhost:${SSE_TEST_PORT}/events`,
        (res) => {
          resolve({
  ...
  test("SSE skips invalid JSON lines", async () => {
    ...
    assert.ok(
      !events.includes("this is not valid json"),
...
describe("SSE connection initialization", () => {
  ...
  test("SSE connection is established with correct headers", async () => {
    const response = await new Promise((resolve, reject) => {
      const req = http.get(
        `http://localhost:${SSE_INIT_PORT}/events`,
        (res) => {
          resolve({
            status: res.statusCode,
            headers: res.headers,
  ...
  test("SSE receives new logs in real-time when starting with no logs", async () => {
    ...
    assert.ok(
      result !== null,
      "should receive the log via SSE when starting with no logs"
    );
    assert.strictEqual(
      result.message,
      testId,
      "received log should have correct message"
    );
  });
});
