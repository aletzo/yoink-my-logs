...
describe("location tracking (server-side)", () => {
  test("log entry includes location with file and line", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 100}`))
      .default;
    ...
    assert.ok(log.location.relativePath, "location should have relativePath");
    assert.ok(log.location.absolutePath, "location should have absolutePath");
    assert.ok(
      typeof log.location.line === "number",
      "location should have line number"
    );
    assert.ok(log.location.line > 0, "line number should be positive");
  });

  test("location.relativePath contains the test filename", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 101}`))
      .default;
    ...
    const log = getLastLog();
    assert.ok(
      log.location.relativePath.includes("location.test.js"),
      `relativePath should include 'location.test.js', got: ${log.location.relativePath}`
    );

    assert.ok(
      log.location.absolutePath.includes("location.test.js"),
      `absolutePath should include 'location.test.js', got: ${log.location.absolutePath}`
    );

    assert.ok(
      log.location.absolutePath.startsWith("/"),
      `absolutePath should be absolute, got: ${log.location.absolutePath}`
    );
  });

  test("tagged methods also include location", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 102}`))
      .default;
    ...
    assert.ok(log.location.relativePath.includes("location.test.js"));
  });

  test("yoink.warn includes location", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 103}`))
      .default;
  ...
  test("yoink.error includes location", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 104}`))
      .default;
  ...
  test("yoink.debug includes location", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 105}`))
      .default;
  ...
  test("yoink.success includes location", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 106}`))
      .default;
  ...
  test("location line number is a reasonable positive integer", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 107}`))
      .default;
  ...
  test("location is present with data-only logs", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 108}`))
      .default;
    ...
    assert.ok(log.location.relativePath);
    assert.ok(log.location.absolutePath);
    assert.ok(log.location.line > 0);
  });

  test("location is present with nested function calls", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 109}`))
      .default;
