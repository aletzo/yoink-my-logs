...
describe("log rotation", () => {
  ...
  test("creating rotated files results in correct enumeration", async () => {
    ...
    fs.writeFileSync(
      file2,
      '{"message":"test2","timestamp":"2025-12-08T00:00:00Z"}\n'
    );
    fs.writeFileSync(
      file3,
      '{"message":"test3","timestamp":"2025-12-08T00:00:01Z"}\n'
    );
  ...
  test("reading all today files returns logs in chronological order", async () => {
    ...
    fs.writeFileSync(
      baseFile,
      '{"message":"first","timestamp":"2025-12-08T00:00:00Z"}\n'
    );
    fs.writeFileSync(
      file2,
      '{"message":"second","timestamp":"2025-12-08T00:00:01Z"}\n'
    );
  ...
  test("MAX_LOG_SIZE constant prevents oversized log entries", async () => {
    const yoink = (await import(`../../index.js?t=${Date.now() + 200}`))
      .default;
  ...
  test("normal sized logs are still written", async () => {
    ...
    assert.strictEqual(
      after,
      before + 1,
      "normal sized log entry should be written"
    );
  });
});
