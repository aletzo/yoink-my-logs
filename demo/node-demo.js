...
yoink.info(
  { version: "1.0.0", nodeVersion: process.version },
...
setTimeout(() => {
  ...
  yoink.debug(
    { query: { page: 1, limit: 10 }, cacheHit: true },
...
setTimeout(() => {
  yoink(
    { orderId: "ORD-001", items: 3, total: 99.99, currency: "USD" },
    "Order processed"
  );
  yoink.success(
    { orderId: "ORD-001", paymentId: "pay_123" },
    "Payment successful"
  );
}, 1500);
...
setTimeout(() => {
  ...
  yoink.warn(
    { endpoint: "/api/v1/users", suggestion: "Use /api/v2/users" },
...
setTimeout(() => {
  yoink.error(
    { code: "ETIMEDOUT", host: "api.example.com", retries: 3 },
    "Connection timeout"
  );
  yoink.error(
